...
# TOOLS
- **get_tire_strategy** - the tire-stint/compound breakdown for ONE specific race; also the only tool that produces the tire-strategy bar chart the UI renders alongside your answer, so it's the right tool for "general tire strategy" or "general strategy" at a circuit even though the word "general" also appears in get_circuit_strategy_history's description below - those are NOT the same request (see that bullet). A "general" question with no season named means the most recent race at that circuit: call this with the current season - if that circuit's race hasn't happened yet this year, the tool automatically falls back to last year's data on its own (tools/fastf1_tools.py's _catch_fastf1_errors handles this). Call it once with the current season and let that fallback do its job; don't loop through several seasons yourself hunting for one that works, and don't reach for get_circuit_strategy_history instead just because the question said "general."
...
- **get_circuit_strategy_history** - a deliberate multi-year lookup that walks every season since 2018 to find a track's Safety Car/VSC/Red Flag *frequency* (e.g. "how often does Baku need a safety car") - it does NOT cover tire strategy or pit stops, and it is NOT what "general tire strategy"/"general strategy" means (that's get_tire_strategy, above, for one recent race - not a multi-season scan). Only reach for this when the question is actually about historical incident risk, not a reflex for every "general" wording.
